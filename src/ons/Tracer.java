 ...
 * the simulation, such as arriving and departing flows, created lightpaths, and
 * accepted or blocked flows.
 *
 * @author andred
 *
 * Trace Format: [event] [time] [info]
 *
 * Events: a = flow arrival d = flow departure n = flow accepted into the
 * network b = flow blocked
 *
 */
public class Tracer {
    ...
    /**
     * Prints formatted representations of a Tracer object to a text-output
     * stream.
     *
    ...
    public void setTraceFile(String filename) throws IOException {
        ...
        System.out.println("Configurando arquivo trace");//TEST
    }
    ...
    /**
     * Adds an object to the trace file. If it is an Event, the addEvent method
     * will deal with it. Otherwise, the object is simply transformed into a
     * String and printed into the trace file.
     *
    ...
    public void acceptBulkData(BulkData bulkData, LightPath[] lightpaths) {
        String str;

        str = "bulkData-accepted " + "- " + bulkData.toTrace();
        for (int i = 0; i < lightpaths.length; i++) {
            str += " " + Long.toString(lightpaths[i].getID());
        }
        if (writeTrace) {
    ...
    public void blockBulkData(BulkData bulkData) {
        if (writeTrace) {
            trace.println("bulkData-call-blocked " + "- " + bulkData.toTrace());
        }
    }
        ...
        try {
            if (event instanceof FlowArrivalEvent) {
                if (writeTrace) {
                    ...
                    System.out.println("Evento FlowArrival setado no trace com sucesso");//TEST
                }
            ...
            if (event instanceof BulkDataArrivalEvent) {
                if (writeTrace) {
                    trace.println("Bulk Data-arrived " + Double.toString(event.getTime()) + " " + ((BulkDataArrivalEvent) event).getBulkData().toTrace());
                    System.out.println("Evento BulkArrival setado no trace com sucesso");//TEST
                }
                ...
                if (writeTrace) {
                    ...
                    for (int i = 0; i < size; i++) {
                        trace.println("Bulk Data-arrived " + Double.toString(event.getTime()) + " " + ((BulkDataArrivalEvent) event).getBulkData().toTrace());
                        System.out.println("Evento BatchArrival setado no trace com sucesso");//TEST
                    }
