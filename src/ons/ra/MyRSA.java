...
public class MyRSA implements RA {
    ...
    public void flowArrival(Flow flow) {
        ...
        for (int i = 0; i < links.length; i++){
            ...
            firstSlot = ((EONLink) cp.getPT().getLink(links[i])).getFirstSlotsAvailable(requiredSlots);
            ...
            EONLightPath lp = new EONLightPath(1, flow.getSource(), flow.getDestination(), links,
                    firstSlot, (firstSlot + requiredSlots -1), 0, ((EONLink) cp.getPT().getLink(links[i])).getSlotSize());
